...
function _change_caps(str, cap_type)
	...
	for _, item in ipairs(temp) do
		if not vim.tbl_contains(title_lowercase, item) then
	...
	for _, word in ipairs(words) do
		...
		elseif last_word then
			...
			if type == "title" then
				...
				if
					not dn_utils.is_table_value(title_lowercase, word)
					and not dn_utils.is_table_value(pseudowords, word)
				then
					word = vim.fn.substitute(word, "\\w\\+", "\\u\\0", "g")
				end
			end
...
---@param path string Path to analyse
---@return string _ Directory path
function dn_utils.parent_dir(path)
	-- process param
	assert(type(path) == "string", "Expected string, got " .. type(path))
	assert(path:len() ~= 0, "Expected path, got a zero-length string")
	-- • handle '~' as shortcut for home directory
...
---@return boolean _ Whether file or directory exists
function dn_utils.path_exists(path)
	assert(type(path) == "string", "Expected string, got " .. type(path))
	assert(path:len() ~= 0, "Expected path, got a zero-length string")
	-- handle '~' as shortcut for home directory
	if path:sub(1, 1) == "~" then
...
---• paths containing "." and ".."
---• path containing symlinks.
---@param dirpath string Directory to be deleted
---@param opts table|nil Optional configuration values:
---• {error} (boolean) Whether to issue fatal
---  error instead of warning for failed deletion
---  operations (optional, delete=false)
---@return boolean _ Whether removal occurred without error
function dn_utils.remove_dir(dirpath, opts)
	-- process dirpath
	assert(type(dirpath) == "string", "Expected string, got " .. type(dirpath))
	assert(dirpath:len() ~= 0, "Expected path, got a zero-length string")
	-- • handle '~' as shortcut for home directory
	if dirpath:sub(1, 1) == "~" then
		dirpath = os.getenv("HOME") .. dirpath:sub(2)
	end
	-- • remove terminal slash
	if dirpath:len() > 1 then
		local last_char = dirpath:sub(-1, -1)
		if last_char == "/" or last_char == "\\" then
			dirpath = dirpath:sub(1, -2)
		end
	end
	-- • make sure we have the correct directory path
	assert(vim.fn.isdirectory(dirpath), "Not a directory: " .. dirpath)
	local realpath = vim.loop.fs_realpath(dirpath)
	assert(realpath ~= nil, "Unable to convert '" .. dirpath .. "' to a real path")
	dirpath = realpath
	-- process opts
	opts = opts or {}
	assert(type(opts) == "table", "Expected table, got " .. type(opts))
	...
	-- functions to delete directory contents recursively
	local _remove_dir
	local _remove_item
	...
	_remove_item = function(_item)
		...
		if not ok then
			no_errors = false
			local errmsg = sf("Unable to delete '%s': %s", _item, err)
			if opts.error then
				error(errmsg)
			else
				dn_utils.warning(errmsg)
			end
		end
	end
	_remove_dir = function(_dir)
		local handle = vim.loop.fs_scandir(_dir)
		assert(handle ~= nil, "got nil handle on dir: " .. _dir)
		local item, item_type = vim.loop.fs_scandir_next(handle)
		while item do
			...
			if item_type == "file" or item_type == "link" or item_type == "directory" then
				_remove_item(item_path)
			end
			item, item_type = vim.loop.fs_scandir_next(handle)
		end
	end -- function
	-- delete contents of specified directory
	_remove_dir(dirpath)
	-- delete specified directory itself
	_remove_item(dirpath)
	-- return error status
	return no_errors
end
...
---@brief ]]
vim.api.nvim_create_user_command("DumbifyQuotes", function()
